...
#' @param true_values A character vector of values to be replaced with `true_number`. Default is `NULL`.
#' @param true_number The numeric value corresponding to `true_values`. Default is `1`.
#' @param false_value A single numeric value for values not in `true_values`. Default is `NULL` (no replacement).
#' @param na_value A numeric value to replace `NA` values. Default is `NA_real_` (no replacement for missing values).
#'
...
#' data <- makenumeric(data, column = "spouse", true_values = c("Living with spouse"), true_number = 1, false_value = 0)
#' print(data)
#'
#' data2 <- data.frame(spouse = c("Living with spouse", NA, "Not living with spouse"))
#' data2 <- makenumeric(data2, column = "spouse", true_values = c("Living with spouse"), true_number = 1, false_value = 0, na_value = -1)
#' print(data2)

makenumeric <- function(data, column, true_values = NULL, true_number = 1, false_value = NULL, na_value = NA_real_) {
  # Check if the column exists
  if (!column %in% colnames(data)) {
    stop("Column not found in the dataset.")
  }

  # Ensure the column is character or coercible to character
  if (!is.character(data[[column]]) && !is.factor(data[[column]])) {
    stop("The specified column must be character or factor.")
  }
  ...
  if (!is.null(true_values)) {
    variable[variable %in% true_values] <- as.character(true_number)
  }
